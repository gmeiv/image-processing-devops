...
def apply_chromatic_aberration(image, shift=5): #Glitch effect by shifting color channels
...
def apply_vignette(image):
    h, w = image.shape[:2]
    k_x = cv2.getGaussianKernel(w, w/2)
    k_y = cv2.getGaussianKernel(h, h/2)

    v_mask = (k_y * k_x.T)
    v_mask = v_mask / v_mask.max()

    for i in range(3):
        image[:, :, i] = image[:, :, i] * v_mask
